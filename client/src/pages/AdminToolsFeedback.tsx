...
export default function AdminToolsFeedback() {
  ...
  if (!user || user.role !== "admin") {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: "#f5f5f7" }}>
        <p className="text-[#6e6e73] text-sm">Accesso riservato agli amministratori.</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen" style={{ background: "#f5f5f7" }}>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {tab === "tools" && (
          <div>
            {/* Pending */}
            <h2 className="text-lg font-bold text-[#1d1d1f] mb-4" style={{ fontFamily: F }}>
              In attesa di approvazione ({pendingTools.length})
            </h2>
            {pendingTools.length === 0 ? (
              <div className="rounded-xl p-8 text-center mb-8" style={{ background: "#ffffff", border: "1px solid rgba(255,255,255,0.06)" }}>
                <p className="text-sm text-sm">Nessun tool in attesa</p>
              </div>
            ) : (
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-[#6e6e73] text-sm mt-2 leading-relaxed">{tool.description}</p>
                        <div className="flex gap-4 mt-2 text-xs text-[#aeaeb2]">
            ...
            {approvedTools.length === 0 ? (
              <div className="rounded-xl p-8 text-center" style={{ background: "#ffffff", border: "1px solid rgba(255,255,255,0.06)" }}>
                <p className="text-sm text-sm">Nessun tool approvato</p>
              </div>
            ) : (
                ...
                {approvedTools.map((tool) => (
                  ...
                  >
                    <div>
                      ...
                      <span className="text-[#aeaeb2] text-xs ml-2">— {tool.featuredDate ?? "non programmato"}</span>
                    </div>
        ...
        {tab === "feedback" && (
          <div>
            ...
            {feedbacks.length === 0 ? (
              <div className="rounded-xl p-8 text-center" style={{ background: "#ffffff", border: "1px solid rgba(255,255,255,0.06)" }}>
                <p className="text-sm text-sm">Nessun feedback ricevuto</p>
              </div>
            ) : (
              <div className="space-y-2">
                {feedbacks.map((fb) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="text-[#1d1d1f] text-sm font-medium">{fb.newsletterDate ?? "—"}</div>
                      {fb.comment && <div className="text-[#6e6e73] text-xs mt-1">{fb.comment}</div>}
                      {fb.email && <div className="text-[#aeaeb2] text-xs mt-1">{fb.email}</div>}
                    </div>
                    <span className="text-xs text-[#aeaeb2]">{new Date(fb.createdAt).toLocaleDateString("it-IT")}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        {tab === "opensource" && (
          <div>
            ...
            {osTools.length === 0 ? (
              <div className="rounded-xl p-8 text-center" style={{ background: "#ffffff", border: "1px solid rgba(255,255,255,0.06)" }}>
                <p className="text-sm text-sm">Nessun tool OS inserito</p>
              </div>
            ) : (
              <div className="space-y-2">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-xs mt-0.5">{tool.description?.slice(0, 80)}...</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      {tool.stars ? <span className="text-xs">⭐ {tool.stars.toLocaleString()}</span> : null}
