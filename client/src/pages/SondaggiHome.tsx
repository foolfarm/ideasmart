/**
 * IDEASMART — Sondaggi & Dati
 * Palette: bianco carta (#faf8f3), inchiostro (#1a1a1a), accento blu sondaggi (#2980b9).
 */
...
function NewsCard({ item, showImage = false }: {
  ...
  return (
    <div className="py-3">
      ...
      <SectionBadge label={item.category || "Sondaggi"} />
      <a href={href} target="_blank" rel="noopener noreferrer">
...
export default function SondaggiHome() {
  const today = useMemo(() => new Date(), []);
  const { data: newsData } = trpc.news.getLatest.useQuery({ limit: 20, section: "sondaggi" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const { data: editorial } = trpc.editorial.getLatest.useQuery({ section: "sondaggi" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const { data: reportageItems } = trpc.reportage.getLatestWeek.useQuery({ section: "sondaggi" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const { data: analyses } = trpc.marketAnalysis.getLatest.useQuery({ section: "sondaggi" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const news = newsData || [];
  ...
  return (
    <>
      <SEOHead
        title="Sondaggi & Dati — IDEASMART"
        description="Sondaggi, ricerche di mercato, opinion poll, data journalism e statistiche. Numeri e tendenze dall'Italia e dal mondo."
        canonical="https://ideasmart.ai/sondaggi"
        ...
        <header className="max-w-6xl mx-auto px-4 pt-6 pb-0">
          ...
          <div className="text-center py-5">
            ...
            <p className="mt-2 text-sm text-[#1a1a1a]/50 uppercase tracking-[0.25em]"
              style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Arial, sans-serif" }}>
              Polls · Survey · Opinion · Data Journalism · Statistiche
            </p>
          </div>
          <Divider thick />
            ...
            <div className="mt-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-0">
                <div className="md:col-span-2 md:pr-8 md:border-r border-[#1a1a1a]">
                  ...
                  <SectionBadge label={heroNews.category || "Sondaggi"} />
                  <a href={heroNews.sourceUrl && heroNews.sourceUrl !== '#' ? heroNews.sourceUrl : `https://www.google.com/search?q=${encodeURIComponent(heroNews.title)}`}
                    target="_blank" rel="noopener noreferrer">
                ...
                <div>
                  ...
                  {editorial.keyTrend && (
                    <div className="mt-3 pl-3 border-l-2" style={{ borderColor: ACCENT }}>
                      ...
                      <p className="text-sm text-[#1a1a1a]/70" style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Georgia, serif" }}>
                        {editorial.keyTrend}
                      </p>
                    </div>
                  )}
            ...
            <div className="py-8 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <h3 className="mt-2 text-2xl font-bold text-[#1a1a1a]"
                  style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', Arial, sans-serif" }}>
                  Ricevi Sondaggi & Dati ogni settimana
                </h3>
                <p className="mt-2 text-sm text-[#1a1a1a]/65"
                  style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Georgia, serif" }}>
                  Polls, survey, opinion e data journalism: i numeri che contano dall'Italia e dal mondo.
                </p>
              </div>
              <div>
                <NewsletterSubscribeForm defaultChannel="sondaggi" accentColor={ACCENT} />
              </div>
            </div>
          </div>
          ...
          <div className="mt-4">
            ...
            <div className="py-4 flex flex-col sm:flex-row items-center justify-between gap-2">
              <p className="text-xs text-[#1a1a1a]/40" style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Arial, sans-serif" }}>
                {`© ${today.getFullYear()} IdeaSmart · Sondaggi & Dati`}
              </p>
              <div className="flex items-center gap-4 flex-wrap justify-center sm:justify-end">
                ...
                <Link href="/gossip"><span className="text-xs hover:underline cursor-pointer" style={{ color: "#9b59b6", fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Arial, sans-serif" }}>Business Gossip</span></Link>
                <Link href="/cybersecurity"><span className="text-xs hover:underline cursor-pointer" style={{ color: "#27ae60", fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Arial, sans-serif" }}>Cybersecurity</span></Link>
