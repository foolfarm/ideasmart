...
export default function SondaggiHome() {
  ...
  const { data: newsData } = trpc.news.getLatest.useQuery({ limit: 20, section: "sondaggi" });
  const { data: editorial } = trpc.editorial.getLatest.useQuery({ section: "sondaggi" });
  const { data: reportageItems } = trpc.reportage.getLatestWeek.useQuery({ section: "sondaggi" });
  const { data: analyses } = trpc.marketAnalysis.getLatest.useQuery({ section: "sondaggi" });
  const news = newsData || [];
