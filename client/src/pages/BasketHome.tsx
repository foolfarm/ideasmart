/**
 * IDEASMART — Basket
 * Palette: bianco carta (#faf8f3), inchiostro (#1a1a2e), accento viola basket (#8e44ad).
 */
...
import { AdBasketTop, AdBasketMid1, AdBasketMid2, AdBasketBottom } from "@/components/AdUnit";
...
function NewsCard({ item, showImage = false }: {
  ...
  return (
    <div className="py-3">
      ...
      <SectionBadge label={item.category || "Basket"} />
      <a href={href} target="_blank" rel="noopener noreferrer">
...
export default function BasketHome() {
  ...
  return (
    <>
      <SEOHead
        title="Basket — IDEASMART"
        description="NBA, Serie A, Eurolega e Nazionale. Tutto il basket italiano e internazionale: risultati, analisi e interviste."
        canonical="https://ideasmart.ai/basket"
      ...
      <div className="min-h-screen" style={{ background: "#faf8f3", color: INK }}>
        <header className="max-w-6xl mx-auto px-4 pt-6 pb-0">
          ...
          <div className="text-center py-5">
            ...
            <p className="mt-2 text-sm text-[#1a1a2e]/50 uppercase tracking-[0.25em]"
              style={{ fontFamily: "'Space Mono', monospace" }}>
              NBA · Serie A · Eurolega · Nazionale · Analisi
            </p>
          </div>
          <Divider thick />
        </header>
        ...
        <main className="max-w-6xl mx-auto px-4 pb-16">
            ...
            <div className="mt-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-0">
                <div className="md:col-span-2 md:pr-8 md:border-r border-[#1a1a2e]">
                  ...
                  <SectionBadge label={heroNews.category || "Basket"} />
                  <a href={heroNews.sourceUrl && heroNews.sourceUrl !== '#' ? heroNews.sourceUrl : `https://www.google.com/search?q=${encodeURIComponent(heroNews.title)}`}
                    target="_blank" rel="noopener noreferrer">
          ...
          <AdBasketBottom className="mt-8 mb-2" />
            ...
            <div className="py-8 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <h3 className="mt-2 text-2xl font-bold text-[#1a1a2e]"
                  style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
                  Ricevi Basket ogni settimana
                </h3>
                <p className="mt-2 text-sm text-[#1a1a2e]/65"
                  style={{ fontFamily: "'Source Serif 4', Georgia, serif" }}>
                  NBA, Serie A, Eurolega e Nazionale: le notizie più importanti per gli appassionati di basket.
                </p>
              </div>
              <div>
                <NewsletterSubscribeForm defaultChannel="basket" accentColor={ACCENT} />
              </div>
            </div>
          </div>

          <ArchiveSection section="basket" accentColor={ACCENT} skipCount={10} />

          <div className="mt-4">
            <Divider thick />
            <div className="py-4 flex flex-col sm:flex-row items-center justify-between gap-2">
              <p className="text-xs text-[#1a1a2e]/40" style={{ fontFamily: "'Space Mono', monospace" }}>
                {`© ${today.getFullYear()} IdeaSmart · Basket`}
              </p>
