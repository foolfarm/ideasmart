/**
 * IDEASMART — Motori
 * Palette: bianco carta (#faf8f3), inchiostro (#1a1a1a), accento arancione motori (#e67e22).
 */
...
function NewsCard({ item, showImage = false }: {
  ...
  return (
    <div className="py-3">
      ...
      <SectionBadge label={item.category || "Motori"} />
      <a href={href} target="_blank" rel="noopener noreferrer">
...
export default function MotoriHome() {
  const today = useMemo(() => new Date(), []);
  const { data: newsData } = trpc.news.getLatest.useQuery({ limit: 20, section: "motori" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const { data: editorial } = trpc.editorial.getLatest.useQuery({ section: "motori" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const { data: reportageItems } = trpc.reportage.getLatestWeek.useQuery({ section: "motori" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const { data: analyses } = trpc.marketAnalysis.getLatest.useQuery({ section: "motori" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const news = newsData || [];
  ...
  return (
    <>
      <SEOHead
        title="Motori — IDEASMART"
        description="Auto, moto, Formula 1, mercato automotive e mobilità elettrica. Notizie e analisi dal mondo dei motori."
        canonical="https://ideasmart.ai/motori"
        ...
        <header className="max-w-6xl mx-auto px-4 pt-6 pb-0">
          ...
          <div className="text-center py-5">
            ...
            <p className="mt-2 text-sm text-[#1a1a1a]/50 uppercase tracking-[0.25em]"
              style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Arial, sans-serif" }}>
              Auto · Moto · Formula 1 · Elettrico · Mercato
            </p>
          </div>
          <Divider thick />
            ...
            <div className="mt-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-0">
                <div className="md:col-span-2 md:pr-8 md:border-r border-[#1a1a1a]">
                  ...
                  <SectionBadge label={heroNews.category || "Motori"} />
                  <a href={heroNews.sourceUrl && heroNews.sourceUrl !== '#' ? heroNews.sourceUrl : `https://www.google.com/search?q=${encodeURIComponent(heroNews.title)}`}
                    target="_blank" rel="noopener noreferrer">
            ...
            <div className="py-8 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <h3 className="mt-2 text-2xl font-bold text-[#1a1a1a]"
                  style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', Arial, sans-serif" }}>
                  Ricevi Motori ogni settimana
                </h3>
                <p className="mt-2 text-sm text-[#1a1a1a]/65"
                  style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Georgia, serif" }}>
                  Auto, moto, Formula 1 e mercato automotive: le notizie più importanti per appassionati e professionisti.
                </p>
              </div>
              <div>
                <NewsletterSubscribeForm defaultChannel="motori" accentColor={ACCENT} />
              </div>
            </div>
          </div>

          <ArchiveSection section="motori" accentColor={ACCENT} skipCount={10} />
          ...
          <div className="mt-4">
            ...
            <div className="py-4 flex flex-col sm:flex-row items-center justify-between gap-2">
              <p className="text-xs text-[#1a1a1a]/40" style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Arial, sans-serif" }}>
                {`© ${today.getFullYear()} IdeaSmart · Motori`}
              </p>
              <div className="flex items-center gap-4 flex-wrap justify-center sm:justify-end">
                ...
                <Link href="/news"><span className="text-xs hover:underline cursor-pointer" style={{ color: "#c0392b", fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Arial, sans-serif" }}>News</span></Link>
                <Link href="/tennis"><span className="text-xs hover:underline cursor-pointer" style={{ color: "#27ae60", fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Arial, sans-serif" }}>Tennis</span></Link>
                <Link href="/basket"><span className="text-xs hover:underline cursor-pointer" style={{ color: "#8e44ad", fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Arial, sans-serif" }}>Basket</span></Link>
