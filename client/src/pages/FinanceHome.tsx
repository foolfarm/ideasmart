/**
 * IDEASMART — Finance & Markets
 * Palette: bianco carta (#faf8f3), inchiostro (#1a1a1a), accento verde finanza (#1a1a1a).
 */
...
export default function FinanceHome() {
  ...
  const { data: newsData } = trpc.news.getLatest.useQuery({ limit: 20, section: "finance" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const { data: editorial } = trpc.editorial.getLatest.useQuery({ section: "finance" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const { data: startupData } = trpc.startupOfDay.getLatest.useQuery({ section: "finance" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const { data: reportageItems } = trpc.reportage.getLatestWeek.useQuery({ section: "finance" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const { data: analyses } = trpc.marketAnalysis.getLatest.useQuery({ section: "finance" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const news = newsData || [];
  ...
  return (
    <>
      <SEOHead
        title="Finance & Markets — IDEASMART"
        description="Analisi quotidiana dei mercati finanziari, macro-economia e finanza d'impresa. Dati da McKinsey, FT, Bloomberg, BCE."
        canonical="https://ideasmart.ai/finance"
        ...
        <header className="max-w-6xl mx-auto px-4 pt-6 pb-0">
          ...
          <div className="text-center py-5">
            ...
            <p className="mt-2 text-sm text-[#1a1a1a]/50 uppercase tracking-[0.25em]"
              style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Arial, sans-serif" }}>
              Finance & Markets · Analisi Mercati · Macro · Fintech
            </p>
          </div>
          <Divider thick />
            ...
            <div className="mt-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                  ...
                  {editorial.keyTrend && (
                    <div className="mt-3 pl-3 border-l-2 border-[#1a1a1a]">
            ...
            <div className="py-8 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <h3 className="mt-2 text-2xl font-bold text-[#1a1a1a]"
                  style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', Arial, sans-serif" }}>
                  Ricevi Finance & Markets ogni settimana
                </h3>
                <p className="mt-2 text-sm text-[#1a1a1a]/65"
                  style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Georgia, serif" }}>
                  Analisi di mercato, macro-economia e finanza d'impresa per CFO e investitori italiani.
                </p>
              </div>
              <div>
                <NewsletterSubscribeForm defaultChannel="finance" accentColor="#1a56db" />
              </div>
            </div>
          </div>
          ...
          <div className="mt-4">
            ...
            <div className="py-4 flex flex-col sm:flex-row items-center justify-between gap-2">
              <p className="text-xs text-[#1a1a1a]/40" style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Arial, sans-serif" }}>
                {`© ${today.getFullYear()} IdeaSmart · Finance & Markets`}
              </p>
