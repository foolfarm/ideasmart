...
export default function ReportageDetail() {
    ...
    return (
      <div className="min-h-screen bg-[#f9f6f0] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="font-mono text-sm text-[#1a2744]/60 tracking-widest uppercase">Caricamento reportage…</p>
        </div>
      </div>
    );
  }
  ...
  if (!rep) {
    return (
      <div className="min-h-screen bg-[#f9f6f0] flex items-center justify-center">
        <div className="text-center">
          <p className="font-mono text-sm text-[#1a2744]/60 tracking-widest uppercase mb-4">Reportage non trovato</p>
    ...
    <div className="min-h-screen bg-[#f9f6f0]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <article className="max-w-4xl mx-auto px-4 pb-16">
        ...
        <div className="mb-4 flex items-center gap-3">
          ...
          <span className="font-mono text-xs text-[#1a2744]/50 tracking-widest uppercase">{rep.category}</span>
          <span className="font-mono text-xs text-[#1a2744]/40">Settimana {rep.weekLabel}</span>
        </div>
        ...
        {/* Statistiche */}
        {stats.length > 0 && (
          <div className="grid grid-cols-3 gap-4 mb-8 p-6 border border-[#1a2744]/10 bg-white">
            {stats.map((stat, i) => stat && (
              <div key={i} className="text-center">
                <p className="text-3xl font-bold" style={{ color: cfg.color, fontFamily: "'Playfair Display', serif" }}>{stat.value}</p>
                <p className="font-mono text-xs text-[#1a2744]/60 tracking-widest uppercase mt-1">{stat.label}</p>
              </div>
            ))}
          </div>
        )}
        ...
        {/* Corpo del testo */}
        <div className="text-[#1a2744]/85 leading-relaxed mb-8">
          {rep.bodyText.split('\n\n').map((paragraph, i) => (
        ...
        {rep.quote && (
          <blockquote className="my-8 border-l-4 pl-6 py-2" style={{ borderColor: cfg.color }}>
            <p className="text-xl italic text-[#1a2744]/80 leading-relaxed" style={{ fontFamily: "'Playfair Display', serif" }}>
              "{rep.quote}"
            </p>
          </blockquote>
        )}
        ...
        {features.length > 0 && (
          <div className="mb-8">
            <h3 className="font-mono text-xs tracking-widest uppercase text-[#1a2744]/60 mb-4">Punti chiave</h3>
            <ul className="space-y-3">
              {features.map((f, i) => (
                <li key={i} className="flex items-start gap-3">
                  ...
                  <span className="text-[#1a2744]/80 leading-relaxed">{f}</span>
                </li>
              ))}
            </ul>
          </div>
