        ...
        <div className="relative overflow-hidden rounded-lg bg-[#f5f5f5]" style={{ aspectRatio: "16/9" }}>
          {item.imageUrl ? (
            ...
            <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#f5f5f5] to-[#e8e8e8]">
              <span className="text-[#bbb] text-xs uppercase tracking-widest" style={{ fontFamily: FONT_SANS }}>Startup</span>
            </div>
          )}
      ...
      <Link href={href}>
        <div className="overflow-hidden rounded-md bg-[#f5f5f5]" style={{ aspectRatio: "16/9" }}>
          {item.imageUrl ? (
            ...
            <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#f5f5f5] to-[#e8e8e8]">
              <span className="text-[#bbb] text-[10px] uppercase tracking-widest" style={{ fontFamily: FONT_SANS }}>Startup</span>
            </div>
          )}
        </div>
      </Link>
...
export default function StartupHome() {
  ...
  const { data: newsData } = trpc.news.getLatest.useQuery({ limit: 20, section: "startup" }, queryOpts);
  const { data: editorial } = trpc.editorial.getLatest.useQuery({ section: "startup" }, queryOpts);
  const { data: startupData } = trpc.startupOfDay.getLatest.useQuery({ section: "startup" }, queryOpts);
  const { data: reportageItems } = trpc.reportage.getLatestWeek.useQuery({ section: "startup" }, queryOpts);
  const { data: analyses } = trpc.marketAnalysis.getLatest.useQuery({ section: "startup" }, queryOpts);
  ...
  return (
    <>
      <SEOHead
        ...
        description="Notizie sulle startup italiane ed europee. Funding, founder, innovazione e venture capital. Italia First."
      ...
      <div className="min-h-screen" style={{ background: "#ffffff" }}>
        ...
        <main className="max-w-[1280px] mx-auto px-4 pb-16">
          ...
          <section className="mt-6">
            <SectionHeader title="Startup News" accent={ACCENT} />
          ...
          {grid3.length > 0 && (
            <section className="mt-10">
              <SectionHeader title="Ultime Notizie Startup" accent={ACCENT} />
          ...
          {startupData && (
            <section className="mt-10">
              <SectionHeader title="Startup del Giorno" accent={ACCENT} />
              <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-8 border border-[#e8e8e8] rounded-lg p-6">
                <div>
                  ...
                  <p className="text-[14px] italic text-[#666] mt-1" style={{ fontFamily: FONT_SANS }}>{startupData.tagline}</p>
                  <p className="mt-3 text-[14px] leading-relaxed text-[#444]" style={{ fontFamily: FONT_SANS }}>{startupData.description}</p>
          ...
          {reportageItems && reportageItems.length > 0 && (
            <section className="mt-10">
              <SectionHeader title="Reportage della Settimana" accent={ACCENT} />
          ...
          {analyses && analyses.length > 0 && (
            <section className="mt-10">
              <SectionHeader title="Analisi di Mercato" accent={ACCENT} />
          ...
          {listNews.length > 0 && (
            <section className="mt-10">
              <SectionHeader title="Altre Notizie Startup" accent={ACCENT} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <p className="mt-2 text-[14px] text-[#666]" style={{ fontFamily: FONT_SANS }}>
                  Le startup italiane ed europee più interessanti, ogni lunedì.
                </p>
              </div>
              <div>
          ...
          <ArchiveSection section="startup" accentColor={ACCENT} skipCount={10} />
          <SharedPageFooter />
        </main>
      </div>
    </>
