...
export default function SportHome() {
  ...
  const { data: newsData } = trpc.news.getLatest.useQuery({ limit: 20, section: "sport" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const { data: editorial } = trpc.editorial.getLatest.useQuery({ section: "sport" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const { data: startupData } = trpc.startupOfDay.getLatest.useQuery({ section: "sport" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const { data: reportageItems } = trpc.reportage.getLatestWeek.useQuery({ section: "sport" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const { data: analyses } = trpc.marketAnalysis.getLatest.useQuery({ section: "sport" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const news = newsData || [];
  ...
  return (
    <>
      <SEOHead
        title="Sport & Business — IDEASMART"
        description="Business dello sport, diritti TV, valutazioni club e sponsorship. Dati da Deloitte, KPMG, Forbes Sports."
        canonical="https://ideasmart.ai/sport"
        ...
        <header className="max-w-6xl mx-auto px-4 pt-6 pb-0">
          ...
          <div className="text-center py-5">
            ...
            <p className="mt-2 text-sm text-[#1a1a1a]/50 uppercase tracking-[0.25em]"
              style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Arial, sans-serif" }}>
              Sport & Business · Analisi Mercati · Macro · Fintech
            </p>
          </div>
          <Divider thick />
            ...
            <div className="py-8 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <h3 className="mt-2 text-2xl font-bold text-[#1a1a1a]"
                  style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', Arial, sans-serif" }}>
                  Ricevi Sport & Business ogni settimana
                </h3>
                <p className="mt-2 text-sm text-[#1a1a1a]/65"
                  style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Georgia, serif" }}>
                  Business dello sport, deal economici e innovazione sportiva per manager e investitori.
                </p>
              </div>
              <div>
          ...
          <div className="mt-4">
            ...
            <div className="py-4 flex flex-col sm:flex-row items-center justify-between gap-2">
              <p className="text-xs text-[#1a1a1a]/40" style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Arial, sans-serif" }}>
                {`© ${today.getFullYear()} IdeaSmart · Sport & Business`}
              </p>
