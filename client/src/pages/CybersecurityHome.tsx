...
export default function CybersecurityHome() {
  ...
  const { data: newsData } = trpc.news.getLatest.useQuery({ limit: 20, section: "cybersecurity" });
  const { data: editorial } = trpc.editorial.getLatest.useQuery({ section: "cybersecurity" });
  const { data: reportageItems } = trpc.reportage.getLatestWeek.useQuery({ section: "cybersecurity" });
  const { data: analyses } = trpc.marketAnalysis.getLatest.useQuery({ section: "cybersecurity" });
  const news = newsData || [];
