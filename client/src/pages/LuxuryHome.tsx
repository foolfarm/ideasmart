/**
 * IDEASMART — Lifestyle & Luxury
 * Palette: bianco carta (#faf8f3), inchiostro (#1a1a1a), accento verde finanza (#2a2a2a).
 */
...
export default function LuxuryHome() {
  ...
  const { data: newsData } = trpc.news.getLatest.useQuery({ limit: 20, section: "luxury" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const { data: editorial } = trpc.editorial.getLatest.useQuery({ section: "luxury" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const { data: startupData } = trpc.startupOfDay.getLatest.useQuery({ section: "luxury" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const { data: reportageItems } = trpc.reportage.getLatestWeek.useQuery({ section: "luxury" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const { data: analyses } = trpc.marketAnalysis.getLatest.useQuery({ section: "luxury" }, { staleTime: 5 * 60 * 1000, refetchOnWindowFocus: false });
  const news = newsData || [];
  ...
  return (
    <>
      <SEOHead
        title="Lifestyle & Luxury — IDEASMART"
        description="Moda, lusso, made in Italy e lifestyle. Analisi da Bain-Altagamma, McKinsey Luxury, Business of Fashion."
        canonical="https://ideasmart.ai/luxury"
        ...
        <header className="max-w-6xl mx-auto px-4 pt-6 pb-0">
          ...
          <div className="text-center py-5">
            ...
            <p className="mt-2 text-sm text-[#1a1a1a]/50 uppercase tracking-[0.25em]"
              style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Arial, sans-serif" }}>
              Lifestyle & Luxury · Analisi Mercati · Macro · Fintech
            </p>
          </div>
          <Divider thick />
            ...
            <div className="mt-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                  ...
                  {editorial.keyTrend && (
                    <div className="mt-3 pl-3 border-l-2 border-[#2a2a2a]">
                      <p className="text-xs font-bold uppercase tracking-widest text-[#2a2a2a]"
                        style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Arial, sans-serif" }}>
                        Key Trend
                      </p>
            ...
            <div className="py-8 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <h3 className="mt-2 text-2xl font-bold text-[#1a1a1a]"
                  style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', Arial, sans-serif" }}>
                  Ricevi Lifestyle & Luxury ogni settimana
                </h3>
                <p className="mt-2 text-sm text-[#1a1a1a]/65"
                  style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Georgia, serif" }}>
                  Moda, lusso e made in Italy per imprenditori del settore e investitori.
                </p>
              </div>
              <div>
                <NewsletterSubscribeForm defaultChannel="luxury" accentColor="#2a2a2a" />
              </div>
            </div>
          </div>
          ...
          <div className="mt-4">
            ...
            <div className="py-4 flex flex-col sm:flex-row items-center justify-between gap-2">
              <p className="text-xs text-[#1a1a1a]/40" style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Arial, sans-serif" }}>
                {`© ${today.getFullYear()} IdeaSmart · Lifestyle & Luxury`}
              </p>
              <div className="flex items-center gap-4 flex-wrap justify-center sm:justify-end">
                ...
                <Link href="/luxury"><span className="text-xs hover:underline cursor-pointer" style={{ color: "#2a2a2a", fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Helvetica Neue', Arial, sans-serif" }}>Luxury</span></Link>
