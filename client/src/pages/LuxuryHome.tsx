...
export default function LuxuryHome() {
  ...
  const { data: newsData } = trpc.news.getLatest.useQuery({ limit: 20, section: "luxury" });
  const { data: editorial } = trpc.editorial.getLatest.useQuery({ section: "luxury" });
  const { data: startupData } = trpc.startupOfDay.getLatest.useQuery({ section: "luxury" });
  const { data: reportageItems } = trpc.reportage.getLatestWeek.useQuery({ section: "luxury" });
  const { data: analyses } = trpc.marketAnalysis.getLatest.useQuery({ section: "luxury" });
  const news = newsData || [];
