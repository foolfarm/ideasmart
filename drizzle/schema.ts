...
// ── Notification Preferences (preferenze notifiche per iscritti) ─────────────
export const notificationPreferences = mysqlTable("notification_preferences", {
  id: int("id").autoincrement().primaryKey(),
  email: varchar("email", { length: 320 }).notNull().unique(),
  name: varchar("name", { length: 255 }),
  // Canali di notifica
  notifyNews: boolean("notifyNews").default(true).notNull(),         // news giornaliere
  notifyEditorial: boolean("notifyEditorial").default(true).notNull(), // editoriale
  notifyStartup: boolean("notifyStartup").default(true).notNull(),    // startup del giorno
  notifyReportage: boolean("notifyReportage").default(false).notNull(), // reportage settimanale
  notifyMarket: boolean("notifyMarket").default(false).notNull(),     // analisi di mercato
  // Frequenza
  frequency: mysqlEnum("frequency", ["daily", "weekly", "realtime"]).default("daily").notNull(),
  // Categorie di interesse
  categories: text("categories"),  // JSON array: ["AI & Fintech", "AI & Salute", ...]
  // Token per gestione preferenze senza login
