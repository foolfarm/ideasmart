...
// ── News Items ───────────────────────────────────────────────────────────────
export async function getLatestNews(limit = 20, section: 'ai' | 'music' | 'startup' = 'ai') {
  const db = await getDb();
  if (!db) return [];
...
// ── Daily Editorial ──────────────────────────────────────────────────────────────────────────────────
export async function getLatestEditorial(section: 'ai' | 'music' | 'startup' = 'ai') {
  const db = await getDb();
  if (!db) return null;
...
export async function getTodayEditorial(dateLabel: string, section: 'ai' | 'music' | 'startup' = 'ai') {
  const db = await getDb();
  if (!db) return null;
  const result = await db.select().from(dailyEditorial)
...
// ── Startup of the Day ──────────────────────────────────────────────────────────────────────────────
export async function getLatestStartupOfDay(section: 'ai' | 'music' | 'startup' = 'ai') {
  const db = await getDb();
  if (!db) return null;
...
export async function getTodayStartup(dateLabel: string, section: 'ai' | 'music' | 'startup' = 'ai') {
  const db = await getDb();
  if (!db) return null;
  const result = await db.select().from(startupOfDay)
...
export async function getLatestWeeklyReportage(section: 'ai' | 'music' | 'startup' = 'ai') {
  const db = await getDb();
  if (!db) return [];
...
// ── Market Analysis ─────────────────────────────────────────────────────────────────────────────────────────────
export async function getLatestMarketAnalysis(section: 'ai' | 'music' | 'startup' = 'ai') {
  const db = await getDb();
  if (!db) return [];
 ...
 */
export async function getLatestNewsFiltered(limit = 20, section: 'ai' | 'music' | 'startup' = 'ai') {
  const db = await getDb();
  if (!db) return [];
 ...
 */
export async function countFilteredNews(section: 'ai' | 'music' | 'startup' = 'ai') {
  const db = await getDb();
 ...
 */
export async function getLowScoreNews(section: 'ai' | 'music' | 'startup' = 'ai') {
  const db = await getDb();
  if (!db) return [];
