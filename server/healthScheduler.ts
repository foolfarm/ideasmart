/**
 * Health Scheduler — Health & Markets by IDEASMART
 * Genera contenuti per la sezione /health: news, editoriale, reportage, analisi di mercato
 * Tono: senior analyst Nature/NEJM/a16z Bio — dati precisi, framing strategico, impatto per le imprese
 */
...
// ─── Genera 20 news Health ───────────────────────────────────────────────────
export async function generateHealthNews(): Promise<void> {
  console.log("[HealthScheduler] Generating health news...");
  try {
    ...
    const response = await invokeLLM({
      messages: [
        {
          role: "system",
          content: `Sei un senior analyst di Health & Markets by IDEASMART, testata giornalistica HumanLess italiana.
Scrivi in italiano con tono da senior analyst Nature/NEJM/a16z Bio: dati precisi, framing strategico, implicazioni per professionisti sanitari e investitori biotech italiani.
Genera notizie credibili e realistiche sui salute, biotech e medicina.`,
        },
        {
          role: "user",
...
Categorie da usare: Biotech & Pharma, AI in Medicina, Longevità & Aging, Oncologia, MedTech & Dispositivi, Salute Digitale, Ricerca Clinica, Sanità Pubblica, Genomica, Neuroscienze, Drug Discovery, Regolamentazione FDA/EMA.
  ...
  "news": [
    {
      ...
      "sourceUrl": "URL homepage del sito sorgente (es. https://www.nature.com, https://www.statnews.com, https://www.fiercebiotech.com) — NON inventare URL di articoli specifici",
    ...
    await db.delete(newsItems).where(eq(newsItems.section, 'health'));
    ...
    console.log(`[HealthScheduler] Saved ${Math.min(content.news.length, 20)} health news`);
  } catch (err) {
    console.error("[HealthScheduler] Error generating health news:", err);
  }
}

// ─── Genera Editoriale Health ────────────────────────────────────────────────
export async function generateHealthEditorial(): Promise<void> {
  console.log("[HealthScheduler] Generating health editorial...");
  try {
    const today = new Date().toLocaleDateString("it-IT", {
      weekday: "long", day: "numeric", month: "long", year: "numeric",
    });
    ...
    const response = await invokeLLM({
      messages: [
        {
          role: "system",
          content: `Sei il direttore editoriale di Health & Markets by IDEASMART.
Il tuo stile è quello di un senior analyst Nature/NEJM/a16z Bio: parti da un dato di mercato specifico e verificabile, analizza le implicazioni strategiche per professionisti sanitari e investitori biotech italiani, concludi con una provocazione per i peer.
...
Scrivi in italiano, tono autorevole, frasi concise e incisive.`,
        ...
        {
          role: "user",
          content: `Scrivi l'editoriale di Health & Markets per oggi, ${today}.
Scegli il tema più rilevante tra: approvazioni FDA/EMA, breakthrough scientifici, finanziamenti biotech, AI in medicina, longevità.
Restituisci JSON:
{
  "title": "Titolo editoriale (max 80 caratteri, incisivo)",
  "subtitle": "Sottotitolo analitico (max 120 caratteri)",
  "body": "Corpo dell'editoriale (400-600 parole). Struttura: 1) dato di mercato di apertura con fonte, 2) analisi delle implicazioni per le imprese italiane, 3) confronto con scenario europeo/globale, 4) provocazione finale per professionisti sanitari e investitori biotech.",
  "section": "health"
}`,
        },
      ],
    ...
    const db = await getDb();
    if (!db) throw new Error("DB not available");
    const imageUrl = await findEditorialImage(content.title, "health");
    ...
    console.log("[HealthScheduler] Health editorial saved");
  } catch (err) {
    console.error("[HealthScheduler] Error generating health editorial:", err);
  }
}

// ─── Genera Deal/Azienda della Settimana Health ──────────────────────────────
export async function generateHealthDealOfWeek(): Promise<void> {
  console.log("[HealthScheduler] Generating health deal of week...");
  try {
    const weekLabel = new Date().toISOString().split("T")[0];
    const response = await invokeLLM({
      messages: [
        {
          role: "system",
          content: `Sei un analista M&A e deal tracker per Health & Markets by IDEASMART.
...
Tono: senior analyst con dati precisi e implicazioni strategiche.`,
        ...
        {
          role: "user",
          content: `Genera la scheda "Breakthrough della Settimana" per Health & Markets.
Scegli il deal/evento finanziario più significativo della settimana (approvazione FDA/EMA, trial clinico, acquisizione biotech, breakthrough scientifico).
Restituisci JSON:
{
  ...
  "category": "Categoria (Approvazione, Trial, Acquisizione, Ricerca, Regolamentazione)",
  "source": "Fonte principale (es: Bloomberg, FT, Reuters)"
    ...
    console.log("[HealthScheduler] Health deal of week saved");
  } catch (err) {
    console.error("[HealthScheduler] Error generating health deal of week:", err);
  }
}

// ─── Genera Reportage Health ─────────────────────────────────────────────────
export async function generateHealthReportage(): Promise<void> {
  console.log("[HealthScheduler] Generating health reportage...");
  try {
    const weekLabel = new Date().toISOString().split("T")[0];
    const response = await invokeLLM({
      messages: [
        {
          role: "system",
          content: `Sei un senior analyst di Health & Markets by IDEASMART. Produci reportage approfonditi su aziende, deal e trend finanziari con dati precisi e analisi strategica.`,
        },
        {
          role: "user",
          content: `Genera 4 reportage su aziende o deal finanziari rilevanti questa settimana.
Copri settori diversi (es: azienda biotech, startup medtech, trial clinico, player pharma).
Restituisci JSON:
{
  "reportages": [
    {
      "position": 1,
      "sectionNumber": "01",
      "category": "Categoria (Biotech, Pharma, MedTech, AI-Med, Ricerca)",
    ...
    const db = await getDb();
    if (!db) throw new Error("DB not available");
    await db.delete(weeklyReportage).where(eq(weeklyReportage.section, 'health'));
    ...
    console.log("[HealthScheduler] Health reportage saved");
  } catch (err) {
    console.error("[HealthScheduler] Error generating health reportage:", err);
  }
}

// ─── Genera Analisi di Mercato Health ───────────────────────────────────────
export async function generateHealthMarketAnalysis(): Promise<void> {
  console.log("[HealthScheduler] Generating health market analysis...");
  try {
    const weekLabel = new Date().toISOString().split("T")[0];
    const response = await invokeLLM({
      messages: [
        {
          role: "system",
          content: `Sei un senior analyst di Health & Markets by IDEASMART. Produci analisi di mercato approfondite con dati, trend e prospettive per professionisti sanitari e investitori biotech italiani.`,
        },
        {
          role: "user",
  ...
  "analyses": [
    {
      ...
      "source": "Fonte principale (es: Nature, NEJM, FDA, EMA, a16z Bio)",
    ...
    const db = await getDb();
    if (!db) throw new Error("DB not available");
    await db.delete(marketAnalysis).where(eq(marketAnalysis.section, 'health'));
    ...
    console.log("[HealthScheduler] Health market analysis saved");
  } catch (err) {
    console.error("[HealthScheduler] Error generating health market analysis:", err);
  }
}
