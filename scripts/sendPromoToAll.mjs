/**
 * Script one-shot: invia la newsletter Proof Press Promo a tutti gli iscritti attivi.
 * Usa fetch nativo + SendGrid API v3 direttamente.
 * Legge gli iscritti attivi dal DB tramite query MySQL diretta.
 * 
 * Esecuzione: node scripts/sendPromoToAll.mjs
 */
...
try {
  ...
  // usa variabili d'ambiente di sistema
}
...
// ─── Genera HTML con link disiscrizione personalizzato ───────────────────────
function buildHtml(unsubToken) {
  const unsubUrl = unsubToken
    ? `https://proofpress.ai/unsubscribe?token=${unsubToken}`
...
<table class="container" cellpadding="0" cellspacing="0" border="0" width="640" style="max-width:640px;">
      ...
      <table cellpadding="0" cellspacing="0" border="0" width="100%" style="margin-bottom:20px;">
        <tr>
          ...
          <td valign="top">
            ...
            <p style="margin:0;font-family:'Helvetica Neue',Arial,sans-serif;font-size:14px;color:#555;line-height:1.6;"><em>"Analizza questo report trimestrale e identificami i 3 rischi principali con piano d'azione concreto"</em> &mdash; trasforma 2 ore di analisi in 10 minuti di lavoro ad alto valore.</p>
          </td>
        </tr>
      </table>
      ...
      <table cellpadding="0" cellspacing="0" border="0" width="100%" style="margin-bottom:20px;">
        <tr>
          ...
          <td valign="top">
            ...
            <p style="margin:0;font-family:'Helvetica Neue',Arial,sans-serif;font-size:14px;color:#555;line-height:1.6;"><em>"Sei un esperto di go-to-market B2B SaaS. Costruisci una strategia di lancio per [prodotto] con budget &euro;5.000"</em> &mdash; output concreto e strutturato, non teoria generica.</p>
          </td>
        </tr>
      </table>
      ...
      <table cellpadding="0" cellspacing="0" border="0" width="100%" style="margin-bottom:20px;">
        <tr>
          ...
          <td valign="top">
            ...
            <p style="margin:0;font-family:'Helvetica Neue',Arial,sans-serif;font-size:14px;color:#555;line-height:1.6;"><em>"Scrivi 5 subject line email per [prodotto] usando la formula AIDA, tono professionale ma diretto, max 50 caratteri"</em> &mdash; risultati in 30 secondi invece di 30 minuti.</p>
          </td>
        </tr>
      </table>
      ...
      <table cellpadding="0" cellspacing="0" border="0" width="100%">
        <tr>
          ...
          <td valign="top">
            ...
            <p style="margin:0;font-family:'Helvetica Neue',Arial,sans-serif;font-size:14px;color:#555;line-height:1.6;"><em>"Sei un consulente senior. Analizza questo brief del cliente e identificami le domande che non ha fatto ma che determinano il successo del progetto"</em> &mdash; il prompt che impressiona i clienti.</p>
          </td>
        </tr>
      </table>
    </td>
            ...
            <table cellpadding="0" cellspacing="0" border="0" align="center" style="margin-bottom:16px;">
              <tr>
                <td style="background:#d94f3d;border-radius:6px;">
                  <a href="${campaign.ctaUrl}" style="display:inline-block;background:#d94f3d;color:#fff;font-family:'Helvetica Neue',Arial,sans-serif;font-size:18px;font-weight:700;padding:18px 40px;border-radius:6px;text-decoration:none;">${campaign.ctaText}</a>
                </td>
              </tr>
            </table>
  ...
  <tr>
    <td style="background:#f5f3ef;padding:32px 40px;">
      ...
      <p style="margin:0 0 6px;font-family:'Georgia',serif;font-size:15px;font-weight:700;color:#1a1a1a;">&Egrave; un abbonamento?</p>
      <p style="margin:0 0 20px;font-family:'Helvetica Neue',Arial,sans-serif;font-size:14px;color:#555;line-height:1.6;border-bottom:1px solid #e8e4dc;padding-bottom:16px;">No. Paghi &euro;39 una sola volta e ottieni accesso permanente. Nessun rinnovo, nessuna sorpresa.</p>
      ...
      <p style="margin:0 0 20px;font-family:'Helvetica Neue',Arial,sans-serif;font-size:14px;color:#555;line-height:1.6;border-bottom:1px solid #e8e4dc;padding-bottom:16px;">S&igrave;. Compatibile con ChatGPT (GPT-4o), Claude 3.5, Gemini 1.5 Pro, Perplexity. Pronto all'uso senza modifiche.</p>
      <p style="margin:0 0 6px;font-family:'Georgia',serif;font-size:15px;font-weight:700;color:#1a1a1a;">E se non sono soddisfatto?</p>
      <p style="margin:0;font-family:'Helvetica Neue',Arial,sans-serif;font-size:14px;color:#555;line-height:1.6;">Garanzia soddisfatti o rimborsati 30 giorni. Scrivi a <a href="mailto:info@proofpress.ai" style="color:#d94f3d;">info@proofpress.ai</a>.</p>
    </td>
  </tr>
        ...
        <tr>
          <td colspan="2" style="padding-top:16px;border-top:1px solid #333;">
            <p style="margin:8px 0 0;font-family:'Helvetica Neue',Arial,sans-serif;font-size:11px;color:#555;line-height:1.6;">
              ...
              <a href="${unsubUrl}" style="color:#888;">Annulla iscrizione</a> &middot;
...
// ─── Leggi iscritti attivi dal DB ─────────────────────────────────────────────
console.log("📋 Connessione al database...");
const conn = await mysql.createConnection(DATABASE_URL);

const [rows] = await conn.execute(
  "SELECT email, unsubscribeToken FROM subscribers WHERE status = 'active' ORDER BY id"
);
await conn.end();

const subscribers = rows;
console.log(`📧 Trovati ${subscribers.length} iscritti attivi`);

if (subscribers.length === 0) {
  console.error("❌ Nessun iscritto attivo trovato");
  process.exit(1);
}
...
for (let i = 0; i < subscribers.length; i += BATCH_SIZE) {
  ...
  await Promise.all(
    batch.map(async (sub) => {
      const html = buildHtml(sub.unsubscribeToken);
      const body = {
      ...
      try {
        ...
        if (res.status === 202) {
          ...
          console.error(`  ❌ Errore ${res.status} per ${sub.email}:`, errText.slice(0, 100));
          errors++;
        }
      } catch (err) {
        console.error(`  ❌ Network error per ${sub.email}:`, err.message);
        errors++;
      }
    })
  );
  ...
  console.log(`  ✉️  Inviati ${progress}/${subscribers.length} (errori: ${errors})`);
